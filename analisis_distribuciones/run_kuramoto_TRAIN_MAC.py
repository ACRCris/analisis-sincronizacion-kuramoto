"""
Script: run_kuramoto_TRAIN_MAC.py

VERSIÓN ADAPTADA PARA APPLE M3 (MPS/Metal)

OPCIÓN C: TRAINING SET COMPLETO (60,000 imágenes)
VERSIÓN CORREGIDA - Sin promedios prematuros

CARACTERÍSTICAS MAC:
- Usa MPS (Metal Performance Shaders) en lugar de CUDA
- Resultados en carpeta separada: resultados_kuramoto_TRAIN_MAC_60k
- No sobrescribe archivos existentes
- Optimizado para Apple Silicon (M3)
...
- Tiempo estimado: 3-5 horas con Apple M3 GPU
...
def calcular_metricas_imagen_completas(xs, es, idx, label):
    ...
    try:
        ...
        try:
            variance_series = np.var(series, axis=1)
...
def main():
    ...
    kblock = kblock.to(DEVICE)  # Mover modelo al dispositivo (MPS/CUDA/CPU)
    ...
    # Estimación para Apple M3
    print("⏱️  ESTIMACIÓN (Apple M3 con MPS):")
    imgs_restantes = total_images - start_idx
    velocidad_estimada = 3.5  # img/s estimado para Apple M3
    tiempo_segundos = imgs_restantes / velocidad_estimada
    ...
    with tqdm(total=total_images, initial=start_idx, desc="Procesando", 
        ...
        for idx, (image, label) in enumerate(train_loader):
            ...
            # Procesar imagen - mover al dispositivo adecuado
            ...
            # Calcular métricas COMPLETAS (mueve a CPU internamente)
    ...
    velocidad_promedio = len(metricas_acumuladas) / elapsed_total
    ...
    print(f"Velocidad promedio: {velocidad_promedio:.2f} img/s")
    print(f"Dispositivo usado: {DEVICE_NAME}")
    print(f"Archivo final: {final_path}")
    print()
    print("⚠️  IMPORTANTE - METODOLOGÍA SIN PROMEDIOS PREMATUROS:")
    print("   Cada imagen tiene 101 valores de R(t) guardados")
    print("   Total: 6,060,000 observaciones temporales")
    print()
    print("📊 SIGUIENTE PASO:")
    print("   1. Ejecutar: python analizar_distribuciones.py --dataset train_mac")
    print("   2. Verificar normalidad de las 6,060,000 observaciones")
    print("   3. Ejecutar: python analizar_estadisticas_full_dataset.py --dataset train_mac")
    print()
